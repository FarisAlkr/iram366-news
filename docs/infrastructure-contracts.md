...
**Known limitation of the CI guard (worth fixing).** The regex currently matches `^src/payload/migrations/.*\.ts$` only. The actual migration runner is `.sql`-based since PR #10, so the guard's match logic is out of step with reality — a PR adding a real `.sql` migration under `sql/` will fail the guard and require the override token. Either fix the regex to `^src/payload/migrations/sql/.*\.sql$` or fix the runner to also support TS migrations and keep both as valid responses to the guard. Listed in Section 5.
...
The first four rows above are direct corollaries of the five rules. The bottom three are operational footguns that surfaced during real incidents and are worth memorizing on their own.
...
**Migration-guard regex update.** Realign the CI guard in `ci.yml` ("Check for missing migration on schema changes") so it accepts new files under `src/payload/migrations/sql/*.sql`, matching the SQL-runner reality since PR #10. Complexity: trivial — one regex edit. The existing override token (`[skip-migration-check]`) gives current PRs an escape hatch, but the misalignment turns the guard into a friction point rather than a safety net.
