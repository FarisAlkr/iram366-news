...
| Catastrophic data loss — Postgres volume corrupted, accidentally removed (`docker compose down -v`), or VPS rebuilt from scratch with no copy of `/opt/iram366/.env` and the named volume gone                             | The data lives in a single named Docker volume on a single VPS. Without an off-VPS backup the data is unrecoverable.                                                                                                                                                                            | n/a — pre-existing operational risk addressed by the backup pipeline | Nightly `scripts/backup-postgres.sh` runs at 03:00 UTC via cron, dumps Postgres via `docker compose exec db pg_dump`, uploads to R2 at `s3://iram366-media/postgres/{daily,weekly,monthly}/` with tiered retention (7 + 4 + 3 = 14 files at steady state). Restore procedure documented in `docs/restore-from-backup.md` and verified end-to-end against a temp database before shipping (PR #50). |

The first four rows above are direct corollaries of the five rules. The bottom five are operational footguns that surfaced during real incidents or pre-launch hardening and are worth memorizing on their own.
