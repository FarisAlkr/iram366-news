/**
 * CLI seed: bootstraps a fresh database with the admin user, categories,
 * sample articles, and site settings. Idempotent — safe to re-run.
 *
 * Usage:
...
import { ArticleStatus, UserRole } from './domain/enums'
...
const ADMIN_EMAIL = 'iramnews366@gmail.com'
const ADMIN_NAME = 'آدم الطوري'
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          {
            type: 'text',
            text: 'يُعد هذا التقرير جزءاً من تغطية إرم 366 الإخبارية الشاملة للأحداث المحلية والإقليمية، حيث نسعى لتقديم محتوى إخباري موثوق وشامل لقرائنا.',
          },
        ],
      },
    ],
  ...
  for (const cat of CATEGORIES) {
    try {
      ...
      if (isDuplicateError(err)) {
        ...
        if (existing.docs[0]) categoryMap.set(cat.slug, existing.docs[0].id)
  ...
  for (const [catSlug, articles] of Object.entries(SAMPLE_ARTICLES)) {
    ...
    for (let i = 0; i < articles.length; i++) {
      ...
      try {
        ...
        console.log(`✅ Article created: ${article.title.substring(0, 40)}...`)
      } catch (err) {
